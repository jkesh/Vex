...
Interactive routing can be changed without editing the file. Model assignments
are atomically saved in `~/.vex/routing.json`, contain no credentials, and are
loaded by later VEX sessions. These explicit user selections override project
model defaults but remain below process and command-line overrides:
...
/model [provider|query]          repeatedly choose model and target; Esc finishes
...
`/model` is the only interactive model command. It loads every authenticated or keyless Provider concurrently and presents a two-pane selector: Providers remain visible on the left and the active Provider's live model catalog appears on the right. The selector reads raw terminal input directly, including split CSI/SS3 direction keys and SGR mouse packets; both panes accept mouse press and wheel events, keyboard navigation, and type-to-filter. It runs in the terminal's alternate screen and updates only changed rows, so navigation does not append duplicate tables to the session history. Reopening the selector restores assignment marks, adds current or inherited routes to every target, and annotates models with their assigned targets. `/model <query>` filters by Provider metadata or model details instead of silently accepting an arbitrary ID. After model selection, a second picker assigns it to the user default or a specific fixed Agent role. Each assignment is atomically persisted to `~/.vex/routing.json` before the selector returns to the model list, so a later process resolves the same Provider/model routes even when project defaults differ. Catalog failures stay attached to their Provider without hiding successful catalogs, and remote metadata supplies display names, ownership, context limits, and capability hints when available. `/route` follows the same model-first flow, while `/route <role> <provider> <model>` writes the same user routing file and `--model <id>` remains a one-run override for automation. Secrets are never written to routing, run state, or Agent prompts. `VEX_PROVIDER`, `VEX_MODEL`, `VEX_BASE_URL`, `VEX_API_KEY_ENV`, `VEX_PROXY`, and `VEX_MAX_PARALLEL_WRITERS` provide process-level overrides. The non-interactive `vex models [provider]` command remains available for scripts that need a plain-text catalog.
