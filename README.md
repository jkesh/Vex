...
Running `vex` without arguments opens a direct natural-language session. There is no mode-selection screen: each prompt is routed to conversation, read-only technical review, or implementation from its meaning.
...
├──────────────────────────────────────────────────────────────────────┤
│ PROMPT                                                               │
│  Type naturally; auto selects chat, review, or implement.            │
│  Type / for live hints; use Up/Down and Tab to complete.             │
│  /mode auto|chat|review|implement                                    │
...
Short continuation prompts such as `继续` inherit the previously resolved mode. Review reports and model transcripts are stored under `~/.vex/reviews/<workspace-id>/`; review mode does not create a Git repository or change workspace files. Chat history stays in memory for the current VEX process.

Mode calls honor the same independent role routing: the `architect` route supplies chat and ambiguous-intent classification, while the `scout` and `reviewer` routes supply read-only technical review. Configure them permanently under `agents` or temporarily with `/route architect ...`, `/route scout ...`, and `/route reviewer ...`. Implementation continues to use every configured fixed-role route.
...
3. VEX validates the manifest, dependency graph, allowed paths, base commit, commands, and role hashes.
...
8. Findings return to the original owner's existing VEX session and worktree. Verification and review repeat.
...
VEX's native agent runtime implements `read`, `ls`, `find`, `grep`, `bash`, `edit`, `write`, and structural `team_yield`. Read-only roles are checked against Git before and after every turn. Writer edits are checked against manifest paths both at tool time and integration time. Secret files and credential environment variables are hidden from Agent tools; network, destructive, publishing, credential, and direct shell file-writing commands are blocked.
